...
int stats_vow(uint64_t n_cores, bool hansel_gretel, bool collect_stats, uint64_t max_crumbs)
{
    ...
    for (i = 0; i < NUM_INSTS_STATS; i++)
    {
        ...
        printf("\n----------------------------------------------------------------------------------------\n");
        printf("\nInstance:\t");
        printf("n = %" PRIu64 "\t    ", insts_stats[i].NBITS_STATE);
        printf("w = %" PRIu64 "\t", insts_stats[i].MEMORY_LOG_SIZE);
        printf(_ALPHA_CHAR);
        ...
        for (j = 0; j < BENCH_LOOPS; j++)
        {
            ...
            S.PRNG_SEED += (unsigned long)j; /* Different PRNG seed.. */
        ...
        if (collect_stats)
        {
            ...
            printf("Expected number of function versions (n/(2c)): \t%.2f (expected 0.45n/w = %.2f, ratio = %.2f)\n",
                   pow(2, (double)insts_stats[i].NBITS_STATE) / (2 * ((double)dist_cols / (double)random_functions)),
                   0.45 * pow(2, (double)insts_stats[i].NBITS_STATE) / pow(2, (double)insts_stats[i].MEMORY_LOG_SIZE),
                   (pow(2, (double)insts_stats[i].NBITS_STATE) / (2 * ((double)dist_cols / (double)random_functions))) /
                       (0.45 * pow(2, (double)insts_stats[i].NBITS_STATE) / pow(2, (double)insts_stats[i].MEMORY_LOG_SIZE)));
            printf("Expected total run-time (in/(2c)): \t\t%.2f (expected %cn^3/w = %.2f, ratio = %.2f)\n",
                   ((double)number_steps / (double)random_functions) * pow(2, (double)insts_stats[i].NBITS_STATE) / (2 * ((double)dist_cols / (double)random_functions)),
                   251,
                   sqrt(pow(pow(2, (double)insts_stats[i].NBITS_STATE), 3) / pow(2, (double)insts_stats[i].MEMORY_LOG_SIZE)),
                   (((double)number_steps / (double)random_functions) * pow(2, (double)insts_stats[i].NBITS_STATE) / (2 * ((double)dist_cols / (double)random_functions))) / sqrt(pow(pow(2, (double)insts_stats[i].NBITS_STATE), 3) / pow(2, (double)insts_stats[i].MEMORY_LOG_SIZE)));
        }
        else
        {
            ...
            printf("Number of function iterations: \t\t\t%.2f (expected sqrt(n^3/w) = %.2f, ratio = %.2f)\n",
                   ...
                   sqrt(pow(pow(2, (double)insts_stats[i].NBITS_STATE), 3) / pow(2, (double)insts_stats[i].MEMORY_LOG_SIZE)),
                   ((double)number_steps / (double)BENCH_LOOPS) / (sqrt(pow(pow(2, (double)insts_stats[i].NBITS_STATE), 3) / pow(2, (double)insts_stats[i].MEMORY_LOG_SIZE))));
            printf("\tFor collecting dist. points: \t\t%.2f (%.2f%%)\n",
