...
import vow_sidh.sidh_vow as vow
from sidh_vow_instances import load_instance, insts_stats
import pickle
import psutil
import json
import time
...
def memory(
    ...
    f = open('%ssidh_full_atk_%s_hag_%s_precomp_%s'%(server, run_full_atk, hansel_and_gretel, precomputation), 'a')

    insts_stats = load_instance(modulus)

    for inst in insts_stats:
        nbits_state = inst.NBITS_K + 3
...
def main():
    ...
    parser.add_argument("-np", action='store_true', help="disable precomputation (by default precomputation enabled)")
