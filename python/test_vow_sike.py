...
def stats_vow(n_cores, max_crumbs, collect_stats, modulus='p_32_20'):
    ...
    # includes
    from sike_vow_instances import load_instance
    ...
    for i in range(NUM_INSTS_STATS):
        ...
        S = sike_vow.shared_state_t()
        sike_vow.init_shared_state(insts_stats[i], S)

        print "\n----------------------------------------------------------------------------------------\n"
        print "Instance:\t",
        print "e = %ld\t    " % (insts_stats[i].e),
        print "w = %ld\t" % insts_stats[i].MEMORY_LOG_SIZE,
        print "a = %.2f\t" % insts_stats[i].ALPHA,
        print "b = %.2f\t" % insts_stats[i].BETA,
        print "g = %.2f\t" % insts_stats[i].GAMMA,
        print "d = %.2f\t" % insts_stats[i].delta,
        ...
        S.MAX_FUNCTION_VERSIONS = 100000 # Only test a single function
        S.collect_vow_stats = collect_stats
        for j in range(BENCH_LOOPS):
            sike_vow.reset_shared_state(S)
        ...
        if collect_stats:
            ...
            print ""
            print "Expected number of function versions (n/(2c)): \t%.2f" % (1. * 2 ** (insts_stats[i].e-1) / (2. * dist_cols / random_functions)),
            print "(expected 0.45n/w = %.2f," % (0.45 * 2 ** (insts_stats[i].e-1) / 2 ** insts_stats[i].MEMORY_LOG_SIZE),
            print "ratio = %.2f)" % ((1. * 2 ** (insts_stats[i].e-1) / (2. * dist_cols / random_functions)) / (0.45 * 2 ** (insts_stats[i].e-1) / 2 ** insts_stats[i].MEMORY_LOG_SIZE))
            print "Expected total run-time (in/(2c)): \t\t%.2f" % ((1. * number_steps / random_functions) * 2 ** (insts_stats[i].e-1) / ( 2. * dist_cols / random_functions )),
            print "(expected sqrt(n^3/w) = %.2f," % (sqrt( ( (2 ** (insts_stats[i].e-1)) ** 3 ) / (2 ** insts_stats[i].MEMORY_LOG_SIZE) )),
            print "ratio = %.2f)" % (((1. * number_steps / random_functions) * 2 ** (insts_stats[i].e-1) / ( 2. * dist_cols / random_functions )) / sqrt( ( (2 ** (insts_stats[i].e-1)) ** 3 ) / (2 ** insts_stats[i].MEMORY_LOG_SIZE) ))
            ...
            print "(expected sqrt(n^3/w) = %.2f," % (sqrt(((2**((insts_stats[i].e-1)))**3) / (2**(insts_stats[i].MEMORY_LOG_SIZE)))),
            print "ratio = %.2f)" % ((1. * number_steps / BENCH_LOOPS) / (sqrt(((2**((insts_stats[i].e-1)))**3) / (2**(insts_stats[i].MEMORY_LOG_SIZE)))))
            ...
            print "(expected 0.45n/w = %.2f" % (0.45 * 2**(insts_stats[i].e-1) / 2**insts_stats[i].MEMORY_LOG_SIZE),
            print "ratio = %.2f)" % ((1. * random_functions / BENCH_LOOPS) / (0.45 * 2**(insts_stats[i].e-1) / 2**insts_stats[i].MEMORY_LOG_SIZE))
            print "Number of collisions per function: \t\t%.2f" % (1. * collisions / random_functions),
