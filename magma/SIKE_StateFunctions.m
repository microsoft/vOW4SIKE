...
function UpdateState(state,params)
    ...
    h := SHA3_256(hash_in_base)[1..out_bytes];
    h := SequenceToInteger(h,2^8);
    //b := h mod 2; h := h div 2;
    b := 0;
    c := h mod 2; h := h div 2;
    k := h mod 2^(params`e-2);
...
function NewState(state,params)
    state_ := rec< STATE | 
        initial_state := [Random(1)+1,Random(0),Random(2^(params`e-2)-1)],
        current_steps := 0,
        memory := state`memory,
